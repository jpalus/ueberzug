...
Window_create(WindowObject *self) {
    ...
    unsigned long attributes_mask = CWEventMask;
...
Window_finalise(WindowObject *self) {
	if (self->window) {
        set_subscribed_events(
            self->display, self->parent, NoEventMask);
...
Window_init(WindowObject *self, PyObject *args, PyObject *kwds) {
    ...
    Window_create(self);
    set_subscribed_events(
        self->display, self->parent, StructureNotifyMask);
    set_input_mask(
    ...
    switch (event.type) {
        ...
        case ConfigureNotify: {
                ...
                if (delta_width != 0 || delta_height != 0) {
                    ...
                    XResizeWindow(self->display, self->window, self->width, self->height);
                }
...
PyTypeObject WindowType = {
    ...
    .tp_name = "ueberzug.X.Window",
    .tp_doc = 
        "Basic implementation of a overlay window\n"
        "\n"
        "Args:\n"
