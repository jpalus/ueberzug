...
Image_create_shared_image(Image *self) {
    int screen = XDefaultScreen(self->display);
    // Allocate the memory needed for the XImage structure
    self->image = XShmCreateImage(
        self->display, XDefaultVisual(self->display, screen),
        DefaultDepth(self->display, screen), ZPixmap, 0,
    ...
    if (self->image) {
        ...
        XShmAttach(self->display, &self->segmentInfo);
...
Image_destroy_shared_image(Image *self) {
    if (self->image) {
        XShmDetach(self->display, &self->segmentInfo);
...
Image_init(Image *self, PyObject *args, PyObject *kwds) {
    ...
    self->display_pyobject = display_pyobject;
    self->display = ((DisplayObject*)display_pyobject)->display;
...
static PyObject *
Image_copy_to(Image *self, PyObject *args, PyObject *kwds) {
    ...
    gc = XCreateGC(self->display, surface, 0, NULL);
    XShmPutImage(self->display, surface, gc,
                 self->image, 0, 0,
